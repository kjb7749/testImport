...
No Action Required in many cases, check your new patient Web Sched on your web site to see behavior. See online manual for details.";
		...
		private static void To18_2_1() {
			string command;
			DataTable table;
			ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 18.2.1 - Adding wikipage PageContentPlanText"));//No translation in convert script.
			if(DataConnection.DBtype==DatabaseType.MySql) { 
			...
			ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 18.2.1 - Converting WikiPage Content"));//No translation in convert script.
			...
			foreach(DataRow row in tableWikiListAll.Rows) {
				...
				if(titleCur.Length>0 && titleCur.First()!=currentLetter) {
					...
					ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 18.2.1 - Converting WikiPages starting with the letter '"+currentLetter+"'"));//No translation in convert script.
				}
			...
			ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 18.2.1 - Removing Duplicate Custom Dictionary Entries"));//No translation in convert script.
			if(DataConnection.DBtype==DatabaseType.MySql) {
			...
			ODEvent.Fire(new ODEventArgs("ConvertDatabases","Upgrading database to version: 18.2.1"));//No translation in convert script.
